...
; (function () {
     ...
     * Minified by http://www.minifier.org
     */

    if (!Array.from) {
        Array.from = (function () {
            var toStr = Object.prototype.toString; var isCallable = function (fn) { return typeof fn === 'function' || toStr.call(fn) === '[object Function]' }; var toInteger = function (value) {
                var number = Number(value); if (isNaN(number)) { return 0 }
                if (number === 0 || !isFinite(number)) { return number }
                return (number > 0 ? 1 : -1) * Math.floor(Math.abs(number))
            }; var maxSafeInteger = Math.pow(2, 53) - 1; var toLength = function (value) { var len = toInteger(value); return Math.min(Math.max(len, 0), maxSafeInteger) }; return function from(arrayLike) {
    ...
    MagicSearch.prototype = {
        ...
        searchData: function searchData(isAll, isScroll) {
            ...
            if (isAll) {
                ...
                var inputVals = [].concat(_toConsumableArray(new Object(inputVal.toLowerCase().split(' '))));
                var inputData = [];
                ...
                for (var _i4 = 0; _i4 < dataJson.length; _i4++) {
                    ...
                    for (var j = 0; j < options.fields.length; j++) {
                        for (var k = 0; k < inputData.length; k++) {
                            if (dataJson[_i4][options.fields[j]] !== null && dataJson[_i4][options.fields[j]].toLowerCase().indexOf(inputData[k].value) >= 0) {
            ...
            if (data.length === 0) {
                ...
                //delete empty input
                var _inputVals = [].concat(_toConsumableArray(new Object(inputVal.split(' ')))).filter(function (item) {
                    return item !== '';
                });
                var tempArr = [];
